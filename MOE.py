...
class MoELayer(nn.Module):
    ...
    def forward(self, x):
        ...
        # Getting outputs from all experts
        # expert_outputs shape at first is [num_experts, B, expert_output_dim]
...
class MoEAdapterFusionModel(nn.Module):
    ...
    def __init__(self, audio_backbone_dim, video_backbone_dim, pers_backbone_dim,
                 ...
                 fusion_output_dim, # Output dim of MoE layer
    ...
    def forward(self, A_feat, V_feat, P_feat):
        # A_feat, V_feat, P_feat are the outputs from frozen backbones

        # Apply Adapters 
        # Aggregate sequence features before adapter
...
def get_cosine_schedule_with_warmup(optimizer, num_warmup_steps, num_training_steps, num_cycles=0.5, last_epoch=-1):
    ...
    def lr_lambda(current_step):
        ...
        return max(0.0, cosine_decay) # need to ensure non-negative
...
if __name__ == '__main__':
    ...
    # config for paths
    DATA_ROOT_PATH = "./data" 
    MODEL_SAVE_PATH = './best_moe_adapter_fusion_model_v1.pth' 

    # config for data and feature settings
    WINDOW_SPLIT_TIME = 1
    ...
    # config for classification and splitting
    LABEL_COUNT = 2
    TRACK_OPTION = "Track1" 
    VAL_RATIO = 0.1         
    VAL_PERCENTAGE = 0.1    
    ...
    # config for training and optimization
    BATCH_SIZE = 16
    LEARNING_RATE = 1e-4        
    WEIGHT_DECAY = 1e-4
    NUM_EPOCHS_FINAL = 75       
    ...
    # config for loss function and balancing
    FOCAL_LOSS_GAMMA = 3.0
    MANUAL_CLASS_WEIGHTS = [1.0, 2.5] 
    ...
    # code for determining feature dimensions
    ...
    # code for calculating class weights and sampler
    ...
    try:
        moe_model = MoEAdapterFusionModel( 
    ...
    # code for setting up optimizer
    print(f"\n--- Setting up Optimizer (AdamW) ---")
    trainable_params = moe_model.parameters() 
    ...
    # code for setting up loss function
    ...
    # code for setting up learning rate scheduler
