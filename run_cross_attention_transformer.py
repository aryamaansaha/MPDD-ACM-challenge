...
class CrossModalEncoderBlock(nn.Module):
    ...
    def __init__(self, embed_dim, num_heads, ff_dim, dropout):
        super().__init__()
        # Usingg structure (Norm -> Attention/FFN -> Residual)
...
class CrossModalTransformer(nn.Module):
    ...
    def __init__(self, audio_dim, video_dim, pers_dim, embed_dim, nhead, num_cross_modal_layers,
        ...
        # projecting personalized features to embed_dim for final fusion
        self.pers_projection = nn.Linear(hidden_dim_mlp, embed_dim)

        # final fusion and classification head (using concatenation + MLP)
    ...
    def forward(self, A_feat, V_feat, P_feat, A_mask=None, V_mask=None): # Add optional masks
        ...
        # adding Positional Encoding & Dropout
        ...
        for layer in self.cross_modal_layers:
            A_seq, V_seq = layer(A_seq, V_seq) 

        # Aggregating using CLS tokens 
        A_agg = A_seq[:, 0] # [B, D]
        V_agg = V_seq[:, 0] # [B, D]
