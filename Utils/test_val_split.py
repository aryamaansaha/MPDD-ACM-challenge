...
def train_val_split1(file_path, val_ratio=0.1, random_seed=3407):
    ...
    for pen_category, ids in label_to_ids.items():
        ...
        # code for processing label=4 (split in a 2:1 ratio)
        if pen_category == 4:
            for id_ in ids:
        ...
        # code for processing the case of label=3 and id=69
        if pen_category == 3:
            for id_ in ids:
...
def train_val_split2(file_path, val_percentage=0.10, seed=None):
    ...
    for entry in data:
        # code for extracting person ID (assumes format "personID_topicID.npy")
        person_id = entry['audio_feature_path'].split('_')[0]
        grouped_by_person[person_id].append(entry)

    # code for evenly distributing persons based on label category (young dataset is split according to tri_category)
    ...
    # code for calculating the number of persons per category and the number to be selected
    ...
    # code for building the validation set data
    val_data = []
    for entry in data:
        person_id = entry['audio_feature_path'].split('_')[0]
    ...
    # code for building the training set data
    train_data = [entry for entry in data if entry not in val_data]

    # code for counting the total number of tri_category labels in train_data and val_data
    train_category_count = defaultdict(int)
    val_category_count = defaultdict(int)
