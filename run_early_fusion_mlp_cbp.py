...
class FocalLoss(nn.Module):
    ...
    def forward(self, preds, targets):
        ...
        if self.class_weights is not None:
             current_class_weights = self.class_weights.to(preds.device)
    ...
    try:
        ...
        for i in range(num_classes):
            ...
            if count == 0:
                ...
                weight = total_samples / (num_classes * count) # using the inverse frequency
...
if __name__ == '__main__':
    ...
    # code for calculating final weights
    final_labels_np = []
    if getattr(config, 'calculate_weights_dynamically', False):
        try:
            label_key = {2: 'bin_category', 3: 'tri_category', 5: 'pen_category'}.get(config.labelcount)
